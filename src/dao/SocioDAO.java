...
public class SocioDAO {
    ...
    public boolean renovarSocio(int dni) {
        ...
        try (Connection conn = ConexionDB.conectar();
            ...
            if (rs.next()) { fechaVencimientoActualStr = rs.getString("vencimiento"); }
        ...
        } catch (Exception e) { nuevaFecha = LocalDate.now().plusMonths(1); }

        String sqlUpdate = "UPDATE socios SET vencimiento = ? WHERE dni = ?";
        try (Connection conn = ConexionDB.conectar();
             PreparedStatement pstmtUpdate = conn.prepareStatement(sqlUpdate)) {
            pstmtUpdate.setString(1, nuevaFecha.format(formateador));
