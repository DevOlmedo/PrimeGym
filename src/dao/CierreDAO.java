...
public class CierreDAO {
    ...
    // Verifica si el día ya tiene un registro de cierre
    ...
    // --- Obtiene solo el monto en Efectivo ---

    public double getEfectivoPorFecha(LocalDate fecha) {
        String sql = "SELECT efectivo FROM cierres_caja WHERE fecha = ?";
        try (Connection conn = ConexionDB.conectar();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, fecha.toString());
            ResultSet rs = pstmt.executeQuery();
            if (rs.next()) return rs.getDouble("efectivo");
        } catch (SQLException e) {
            System.out.println("Error al obtener efectivo: " + e.getMessage());
        }
        return 0;
    }

    // --- Obtiene solo el monto en Mercado Pago ---

    public double getMercadoPagoPorFecha(LocalDate fecha) {
        String sql = "SELECT mercado_pago FROM cierres_caja WHERE fecha = ?";
        try (Connection conn = ConexionDB.conectar();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, fecha.toString());
            ResultSet rs = pstmt.executeQuery();
            if (rs.next()) return rs.getDouble("mercado_pago");
        } catch (SQLException e) {
            System.out.println("Error al obtener mercado pago: " + e.getMessage());
        }
        return 0;
    }

    // Recupera los datos de un cierre específico para Reportes
